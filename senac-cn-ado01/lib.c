...
int validaentradas(char *argv[], int base1, int argc){
    int numero,total;
	if(argc!=4){
...
void converttobase(char *numero, int base1, int base2, int total){
	int i, cont=0, numerox,  numerotemp=0, verif0=0, verif1=1;
	for(i=total-1;i>=0;i--){
		if(numero[i]>='A' && numero[i]<='Z'){
			numerox = numero[i]-'0'-7;
		}else if (numero[i]>='a' && numero[i]<='z'){
			numerox = numero[i]-'0'-39;
        }
		else{
            numerox = numero[i]-'0';
		}
		
		verif0=numerox*verif1+verif0;
        verif1=verif1*base1;
